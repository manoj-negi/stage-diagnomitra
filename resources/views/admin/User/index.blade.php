                    ...
                    <div class="row">
                        ...
                        <select name="filter" id="" class="form-control mr-1" style="color:#697a8d!important;">
                         ...
                         <input type="" id="search" name="search" placeholder="search" class="form-control" value ="{{(request()->get('search') != null)? request()->get('search'):''}}" placehoder="Search"/>
                                    ...
                                    <a class="form-control  src-btn" href="{{route('hospitals.index')}}"><i class="fa fa-rotate-left"></i></a>
                        ...
                        <tr>
                            ...
                            @forelse($data as $key => $value)
                                <tr data-entry-id="{{ $value->id }}">
                                                ...
                                                <div>
                                                    <span>
                                                        @if(!empty($value->hospital_logo))
                                                            <img src="{{ url('uploads/hospital/'.$value->hospital_logo) }}"
                                    ...
                                    <td>
                                    <form action="{{url('hospital-status-update',$value->id)}}">
                                    <select name="is_approved" class="form-control onChangeStatusSelect" style="color: #697a8d!important;">
                                        <option value="">Select Status</option>
                                        <option value="approved" {{ $value->is_approved == 'approved' ? 'selected' : '' }}>Approved</option>
                                        <option value="pending" {{ $value->is_approved == 'pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="rejected" {{ $value->is_approved == 'rejected' ? 'selected' : '' }}>Rejected</option>
                                    </select>
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-sm" href="{{ route('hospitals.edit', $value->id) }}">
                                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                        </a>
                                    
                                        <form action="{{route('hospitals.destroy',$value->id)}}" method="post" style="display: inline-block;">
                                            @csrf
                                            @method('delete')
