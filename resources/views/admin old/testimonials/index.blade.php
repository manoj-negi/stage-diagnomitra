...
<div class="row pb-2">
    <div class="col-md-12 text-right">
        <a class="btn btn-primary btn-sm my-3" href="{{route('testimonials.create')}}">Create</a>
                    ...
                    <div class="paging-section justify-content-end">
                     <form class="index-form">
                            <div class="search_bar d-flex">
                                ...
                                <a class="form-control src-btn" href="{{route('testimonials.index')}}"><i class="fa fa-rotate-left"></i></a>
                            </div>
                        </form>
                    </div>
                    </div>
                ...
                <table class="table table-responsive tabel-hover">
                    ...
                    <tr>
                        ...
                        <td>{{ucfirst($result->name)}}</td>
                         <td>{{ucfirst($result->designation)}}</td>
                        <td>{{$result->rating??''}} Star</td>
                        <td> @if(strlen($result->review)>=20)

                        {{substr($result->review, 0,  20).'...' ??''}}
                        @else
                        ...
                        <td><a href="{{route('testimonials.edit',$result->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                        <form action="{{route('testimonials.destroy',$result->id)}}" method="post" style="display: inline-block;">
                        @csrf
                        @method('delete')
